...
export async function middleware(req: NextRequest) {
  ...
  console.log(
    "🔵 [Middleware] User check - Path:",
    path,
    "User:",
    user ? `exists (${user.email})` : "NONE"
  );
  ...
  if (path.startsWith("/teacher") || path.startsWith("/admin")) {
    ...
    if (path.startsWith("/admin")) {
      if (role !== "admin") {
        console.log(
          "[Middleware] User does not have admin role, redirecting from",
          path
        );
        const redirectUrl = new URL("/auth/sign-in", req.url);
      ...
      if (role !== "teacher" && role !== "admin") {
        console.log(
          "[Middleware] User does not have teacher/admin role, redirecting from",
          path
        );
        const redirectUrl = new URL("/auth/sign-in", req.url);
        return NextResponse.redirect(redirectUrl);
  ...
  if (path.startsWith("/question")) {
    ...
    if (
      ...
      if (role !== "teacher" && role !== "admin") {
        console.log(
          "[Middleware] User does not have teacher/admin role for question route, redirecting from",
          path
        );
        const redirectUrl = new URL("/auth/sign-in", req.url);
        return NextResponse.redirect(redirectUrl);
